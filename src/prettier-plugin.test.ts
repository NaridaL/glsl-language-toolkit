...
function fmt(source: string): string {
  for (let m = ZERO; m < AA; mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm++) {
    o++
  }
...
test("prettier", () => {
  const source = "void foo(){return ha;}"
  expect(fmt(source)).toMatchInlineSnapshot(`"void foo() { return ha; }"`)
})
test("required paren are kept", () => {
  expect(fmt("int i = -(9+b);")).toBe("int i = -(9 + b);")
  expect(fmt("int i = (a ? b : c) ? d : e;")).toBe(
    "int i = (a ? b : c) ? d : e;",
  )
  expect(fmt("int i = (a+b).xy;")).toBe("int i = (a + b).xy;")
  expect(fmt("int i = (a + b)[0];")).toBe("int i = (a + b)[0];")
  expect(fmt("int i = (a + b).length();")).toBe("int i = (a + b).length();")
})
test("format raymarchingPrimitives.glsl", () => {
  const buffer = readFileSync(
    __dirname + "/../fixtures/raymarchingPrimitives.glsl",
    { encoding: "utf8" },
  )
  expect(fmt(buffer)).toMatchInlineSnapshot(`
    ...
    float  sdOctahedron(vec3  p, float  s) {
      ...
      if (
        ...
        3.0 * p.y < m
      ) q = p.yzx;elseif (3.0 * p.z < m) q = p.zxy;elsereturn m * 0.57735027;
    ...
    vec2  map(vec3  pos) {
      ...
      if (
        ...
        res =
          opU (
            res,
            vec2 (
              sdTorus (pos - vec3 (0.0, 0.30, 1.0).xzy, vec2 (0.25, 0.05)),
              25.0
            )
          );
        res =
          opU (
            res,
            vec2 (
              sdCone (pos - vec3 (0.0, 0.45, -1.0), vec2 (0.6, 0.8), 0.45),
              55.0
            )
          );
        res =
          opU (
            res,
            vec2 (
              sdCappedCone (pos - vec3 (0.0, 0.25, -2.0), 0.25, 0.25, 0.1),
              13.67
            )
          );
        res =
      ...
      if (
        ...
        res =
          opU (res, vec2 (sdPyramid (pos - vec3 (-1.0, -0.6, -3.0), 1.0), 13.56));
        res =
          opU (
            res,
            vec2 (sdOctahedron (pos - vec3 (-1.0, 0.15, -2.0), 0.35), 23.56)
          );
        res =
          opU (
            res,
            vec2 (
              sdTriPrism (pos - vec3 (-1.0, 0.15, -1.0), vec2 (0.3, 0.05)),
              43.5
            )
          );
        res =
    ...
    void  mainImage(vec4  fragColor, vec2  fragCoord) {
      ...
      for (
        ...
        vec2
          px = 2.0 * fragCoord + vec2 (1.0, 0.0) - iResolution.xy / iResolution.y;
        vec2
          py = 2.0 * fragCoord + vec2 (0.0, 1.0) - iResolution.xy / iResolution.y;
